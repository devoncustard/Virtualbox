...
#c:\github\virtualbox\host_scripts\revokecert.ps1 $puppet $environment $p
...
{
    write-output "    config.vm.define :$hostname do |guest|" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.vm.box= ""$boxname""" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.vm.guest = :$boxos" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.windows.halt_timeout=25" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.winrm.username = ""$user""" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.winrm.password=""$password""" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.winrm.max_tries=30" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.winrm.timeout=3600" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.windows.set_work_network" |out-file -encoding ascii -append $VAGRANT_BASE_FOLDER\$environment\$p\vfile
    write-output "        guest.vm.network :forwarded_port, guest:5985, host:5985, id:""winrm"", auto_correct: true" |out-file -encoding ascii -append $vfile
